...
class MyIDF(val minDocFreq: Int) {
   ...
   */
  def fit(dataset: RDD[Vector]): MyIDFModel = {
    val idf = dataset.treeAggregate(new MyIDF.DocumentFrequencyAggregator(
...
class MyIDFModel private[ml] (val idf: Vector) extends Serializable {
